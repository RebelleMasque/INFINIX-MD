...
async function attpCommand(sock, chatId, message) {
    ...
    if (!text) {
        await sock.sendMessage(chatId, { text: 'Veuillez saisir du texte après la commande .attp ➥ .attp salut' }, { quoted: message });
        return;
    }

    try {
        const mp4Buffer = await renderBlinkingVideoWithFfmpeg(text);
        const webpPath = await writeExifVid(mp4Buffer, { packname: '𝐈𝐍𝐅𝐈𝐍𝐈𝐗•𝐌𝐃' });
